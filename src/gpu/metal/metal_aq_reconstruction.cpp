...
void MetalPreparedAqEvaluation::EncodeInvariantColorCorrelation(
    MetalBackend& backend, MTL::ComputeCommandEncoder* encoder) const {
  if (invariant_color_correlation_from_policy_) {
    // Fused initialization has already adjusted the policy field on device.
    // Its first quantizer serves both invariant CfL and reconstruction.
    EncodeResidentQuantizer(backend, encoder);
