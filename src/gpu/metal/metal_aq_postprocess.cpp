...
void MetalPreparedAqEvaluation::EncodePostprocess(
  ...
  if (options_.loop_filter.gaborish) {
  ...
  const uint32_t iterations = options_.loop_filter.epf_options.iterations;
...
Status MetalPreparedAqEvaluation::GetPostprocessPlan(
  ...
  *plan = {
      ...
      .gaborish_dispatches = options_.loop_filter.gaborish ? 1u : 0u,
      .epf_dispatches = options_.loop_filter.epf_options.iterations,
      .color_dispatches = 1,
      .copy_dispatches = 0,
  };
  return Status::Ok();
