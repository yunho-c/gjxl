...
// The CPU-compatible fast CfL path accumulates four SIMD-style lanes. Assign
// one CUDA thread to each lane so all four advance concurrently while retaining
// the per-lane sample order and the final (0 + 1) + (2 + 3) reduction.
constexpr unsigned int kCflLanesPerTile = 4;
constexpr unsigned int kCflThreads = 128;
constexpr unsigned int kCflTilesPerBlock = kCflThreads / kCflLanesPerTile;

static_assert(kCflThreads % kCflLanesPerTile == 0);
...
__global__ void FuzzyErosionKernel(const float* pre_erosion, float* quant_field,
  ...
  if (strategy_mask != nullptr) strategy_mask[index] = strategy;
...
__device__ float HorizontalCflSum(float value) {
  constexpr unsigned int kFullWarpMask = 0xffffffffu;
  const float lane_0 =
      __shfl_sync(kFullWarpMask, value, 0, kCflLanesPerTile);
  const float lane_1 =
      __shfl_sync(kFullWarpMask, value, 1, kCflLanesPerTile);
  const float lane_2 =
      __shfl_sync(kFullWarpMask, value, 2, kCflLanesPerTile);
  const float lane_3 =
      __shfl_sync(kFullWarpMask, value, 3, kCflLanesPerTile);
  return __fadd_rn(__fadd_rn(lane_0, lane_1),
                   __fadd_rn(lane_2, lane_3));
}
...
__global__ void InitialCflKernel(const float* coding_x, const float* coding_y,
                                 ...
                                 CudaAqGeometry geometry) {
  const unsigned int lane = threadIdx.x % kCflLanesPerTile;
  const unsigned int tile =
      blockIdx.x * kCflTilesPerBlock + threadIdx.x / kCflLanesPerTile;
  ...
  float sum_b = 0.0f;
  bool finite = true;
  // Padded image and partial color-tile widths are multiples of eight, so the
  // x-coordinate modulo four is the CPU sample-index lane on every row.
  for (unsigned int y = y_begin; y < y_end; ++y) {
    const size_t row = static_cast<size_t>(y) * geometry.width;
    for (unsigned int x = x_begin + lane; x < x_end;
         x += kCflLanesPerTile) {
  ...
  const unsigned int finite_lanes =
      __ballot_sync(0xffffffffu, finite);
  const unsigned int group_shift = threadIdx.x & 28u;
  const unsigned int group_mask = 0x0fu << group_shift;
  if ((finite_lanes & group_mask) != group_mask) {
    if (lane == 0) atomicOr(error, 1u << 5);
    return;
  }
  const float count = static_cast<float>((x_end - x_begin) *
                                         (y_end - y_begin));
  ...
  for (unsigned int y = y_begin; y < y_end; ++y) {
    ...
    for (unsigned int x = x_begin + lane; x < x_end;
      ...
      linear_b =
          fmaf(a, centered_y - (coding_b[row + x] - mean_b), linear_b);
    }
  }
...
__global__ void EncodeFrameKernel(
    ...
    float x_multiplier, float b_multiplier, float* deferred_dc) {
  ...
  if (thread == 0) {
    ...
    if (deferred_dc != nullptr) {
      deferred_dc[block] = dc_x;
      deferred_dc[block_count + block] = dc_y;
      deferred_dc[2 * block_count + block] = dc_b;
    } else {
...
__global__ void UniformInitialFieldKernel(
    float* quant_field, float* strategy_mask, float* pixel_mask,
    CudaAqGeometry geometry, float quant) {
  const unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
  const unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
  if (x >= geometry.block_width || y >= geometry.block_height) return;
  const size_t index = static_cast<size_t>(y) * geometry.block_width + x;
  quant_field[index] = quant;
  const float mask = 1.0f / (quant + 0.001f);
  if (strategy_mask != nullptr) strategy_mask[index] = mask;
  if (pixel_mask != nullptr) {
    for (unsigned int dy = 0; dy < 8; ++dy) {
      for (unsigned int dx = 0; dx < 8; ++dx) {
        pixel_mask[static_cast<size_t>(8 * y + dy) * geometry.width +
                   8 * x + dx] = mask;
      }
    }
  }
...
cudaError_t LaunchCudaAqUniformInitialField(
    float* quant_field, float* strategy_mask, float* pixel_mask,
    unsigned int* error, CudaAqGeometry geometry, float quant,
    cudaStream_t stream) {
  if (quant_field == nullptr || error == nullptr ||
      geometry.block_width == 0 || geometry.block_height == 0 ||
      geometry.width / 8 != geometry.block_width || geometry.width % 8 != 0 ||
      geometry.height / 8 != geometry.block_height || geometry.height % 8 != 0 ||
      !std::isfinite(quant) || quant <= 0.0f ||
      !(1.0f / (quant + 0.001f) > 0.0f)) return cudaErrorInvalidValue;
...
cudaError_t LaunchCudaAqEncodeFrame(
    ...
    int* quantized_dc, unsigned int* error, CudaAqGeometry geometry,
    float x_matrix_multiplier, float b_matrix_multiplier,
    CudaDcProcessingParams dc_params, bool deferred_dc, cudaStream_t stream) {
  cudaError_t status = cudaMemsetAsync(error, 0, sizeof(*error), stream);
  if (status != cudaSuccess) return status;
  ...
  return LaunchCudaDcQuantization(gathered, quantized_dc, error, dc_params,
                                   quantizer_params, stream);
}
