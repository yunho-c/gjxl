...
__global__ void InitialGradientKernel(
  ...
  for (unsigned int row_index = 0; row_index < 4; ++row_index) {
    ...
    for (unsigned int column = 0; column < 4; ++column) {
      ...
      const float center = coding_y[static_cast<size_t>(y) * geometry.width + x];
      const float base = 0.25f * (
        coding_y[static_cast<size_t>(bottom) * geometry.width + x] +
      ...
      const float delta = GammaRatio(center + kGammaOffset, false) *
        (center - base);
      const float mask = 1.0f / (CompensatedLog1p(fabsf(delta)) + 0.01f);
      pixel_mask[static_cast<size_t>(y) * geometry.width + x] = mask;
      const float difference = MaskingSqrt(fminf(delta * delta, kDifferenceLimit));
...
__device__ void StoreMin4(
  float value, float& a, float& b, float& c, float& d) {
  if (value >= d) return;
  if (value < a) {
    d = c; c = b; b = a; a = value;
  } else if (value < b) {
...
__device__ void Sort4(float& a, float& b, float& c, float& d) {
  ...
  if (a > c) { const float t = a; a = c; c = t; }
  if (a > d) { const float t = a; a = d; d = t; }
  if (b > c) { const float t = b; b = c; c = t; }
  if (b > d) { const float t = b; b = d; d = t; }
  if (c > d) { const float t = c; c = d; d = t; }
}

__device__ float ErodedValue(
  const float* source,
  unsigned int x,
  unsigned int y,
  unsigned int width,
...
__global__ void FuzzyErosionKernel(
  const float* pre_erosion,
  float* quant_field,
  float* strategy_mask,
  unsigned int* error,
  CudaAqGeometry geometry,
  ...
  float value = ErodedValue(pre_erosion, sx, sy, pre_width, pre_height, weights);
  ...
  value += ErodedValue(
    pre_erosion, sx + 1, sy + 1, pre_width, pre_height, weights);
...
__device__ float FastLog2(float value) {
  ...
  const unsigned int mantissa = bits - (static_cast<unsigned int>(exponent) << 23);
...
__device__ float FastPow2(float value) {
  ...
  const float exponent_value = __uint_as_float(static_cast<unsigned int>(exponent) << 23);
...
__global__ void ModulationKernel(
  ...
  for (unsigned int dy = 0; dy < 8; ++dy) {
    const size_t row = static_cast<size_t>(pixel_y + dy) * geometry.width + pixel_x;
    ...
    for (unsigned int dx = 0; dx < 8; ++dx) {
      ...
      gamma[lane] += GammaRatio(biased_y + x, true);
      if (dx + 1 < 8) {
        frequency[lane] += fminf(kFrequencyLimit, fabsf(y - coding_y[row + dx + 1]));
      }
  ...
  const float overall = ((gamma[0] + gamma[1]) + (gamma[2] + gamma[3])) *
    (0.5f / 64.0f);
  const size_t index = static_cast<size_t>(by) * geometry.block_width + bx;
  const float gamma_value = fmaf(
    0.1005613337192697f, FastLog2(overall), ComputeMask(quant_field[index]));
  const float frequency_sum = (frequency[0] + frequency[1]) +
  ...
  const float exponent = fminf(
    frequency_value, gamma_value + blue_sum * 0.90590804735610064f);
...
__global__ void SelectionInitializeKernel(
  unsigned int* state, unsigned int* histogram, unsigned int median_index) {
...
__global__ void SelectionHistogramKernel(
  const float* values,
...
__global__ void SelectionBucketKernel(
  unsigned int* histogram,
  unsigned int* state,
  float* statistics,
  unsigned int shift,
  ...
  if (index == 0) {
    ...
    for (unsigned int bucket = 0; bucket < 256; ++bucket) {
      ...
      if (rank < prefix + count) {
        ...
        state[1] |= 255u << shift;
        state[2] = rank - prefix;
        if (shift == 0) statistics[deviation ? 1 : 0] = __uint_as_float(state[0]);
        break;
      }
      prefix += count;
    }
...
__global__ void FinalizeQuantizerKernel(
  const float* statistics,
  ...
  float scale = 65536.0f * (median - deviation) / 5.0f;
  ...
  if (!isfinite(median) || median <= 0.0f ||
      !isfinite(deviation) || deviation < 0.0f ||
...
__global__ void RawQuantKernel(
  ...
  const float value = quant_field[index] *
    (65536.0f / static_cast<float>(quantizer[0])) + 0.5f;
...
__device__ signed char QuantizeCfl(float value) {
  ...
  if (value >= kTowardsZero) value -= kTowardsZero;
  else if (value <= -kTowardsZero) value += kTowardsZero;
  else value = 0.0f;
  return static_cast<signed char>(fminf(fmaxf(roundf(value), -128.0f), 127.0f));
}

__global__ void InitialCflKernel(
  const float* coding_x,
  const float* coding_y,
  const float* coding_b,
  signed char* y_to_x,
  ...
  if (sample == 0) { atomicOr(error, 1u << 5); return; }
...
__global__ void GatherDct8Kernel(
  const float* x,
  const float* y,
  const float* b,
  float* gathered,
  CudaAqGeometry geometry) {
  const size_t pixel_count = static_cast<size_t>(geometry.width) * geometry.height;
  const size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  if (index >= 3 * pixel_count) return;
  const unsigned int channel = static_cast<unsigned int>(index / pixel_count);
  const size_t channel_index = index - static_cast<size_t>(channel) * pixel_count;
  const unsigned int block = static_cast<unsigned int>(channel_index / 64);
  const unsigned int coefficient = static_cast<unsigned int>(channel_index & 63u);
  ...
  gathered[index] = source[
    static_cast<size_t>(by * 8 + py) * geometry.width + bx * 8 + px];
}

__device__ int QuantizeCoefficient(
  float coefficient,
  float inverse_dequant,
  unsigned int global_scale,
  int raw_quant,
  ...
  if (!isfinite(rounded) || rounded < -2147483648.0f || rounded >= 2147483648.0f) {
    atomicOr(error, 1u << 7);
    return 0;
  }
...
__device__ float DequantizeCoefficient(
  int quantized,
  float dequant,
  unsigned int global_scale,
  int raw_quant,
  ...
  if (!isfinite(coefficient)) { atomicOr(error, 1u << 8); return 0.0f; }
  return coefficient;
}
...
__device__ int AdjustQuantForChannel(
  ...
  for (unsigned int y = 0; y < 8; ++y) {
    for (unsigned int x = 0; x < 8; ++x) {
      ...
      const float value = coefficient * (quant_tables[192 + table] * qac * multiplier);
      if (!isfinite(coefficient) || !isfinite(value)) {
        atomicOr(error, 1u << 9);
        continue;
      }
      const float quantized = fabsf(value) < thresholds[quadrant] ? 0.0f : rintf(value);
      ...
      if (quantized != 0.0f) {
        ...
        if (in_corner || (on_border && in_larger_corner)) border_sum += fabsf(value);
      }
    }
  }
  int raw = initial_raw;
  ...
  (void)error_sum;
  (void)value_sum;
  const float minimum = fminf(fminf(nonzeros[0], nonzeros[1]),
                              fminf(nonzeros[2], nonzeros[3]));
...
__global__ void AdjustQuantKernel(
  const float* coefficients,
  const float* quant_tables,
  int* raw_quant,
  const unsigned int* quantizer,
  ...
  for (unsigned int ci = 0; ci < 3; ++ci) {
    ...
    const float multiplier = channel == 0 ? x_multiplier :
      (channel == 2 ? b_multiplier : 1.0f);
  ...
  for (unsigned int q = 0; q < 4; ++q) y_thresholds[block * 4 + q] = selected_y[q];
}
...
__global__ void EncodeFrameKernel(
  ...
  if (thread == 0) {
    ...
    const float reconstructed_dc_y = static_cast<float>(qy) /
      (512.0f * quant_scale);
    quantized_dc[block] = RoundDc(dc_x * (4096.0f * quant_scale), error);
    quantized_dc[block_count + block] = qy;
    quantized_dc[2 * block_count + block] = RoundDc(
      (dc_b - reconstructed_dc_y) * (256.0f * quant_scale), error);
  }
  ...
  for (unsigned int i = 0; i < 2; ++i) {
    ...
    const size_t offset = static_cast<size_t>(channel) * channel_stride +
      base + coefficient;
    const float predicted = coefficients[offset] - factors[i] * reconstructed_y[coefficient];
...
cudaError_t LaunchCudaAqInitialQuantization(
  ...
  const dim3 gradient_grid(
    (geometry.width / 4 + 15) / 16, (geometry.height / 4 + 15) / 16);
  ...
  const float mix = butteraugli_target < 2.0f
    ? (2.0f - butteraugli_target) * 0.5f : 0.0f;
  float weights[4];
  float sum = 0.0f;
  ...
  const dim3 block_grid(
    (geometry.block_width + 15) / 16, (geometry.block_height + 15) / 16);
  ...
  constexpr float kFilter[5] = {
    0.364911248f, 0.05f, 0.1688888021f, 0.221069183f, 0.306563504f};
  constexpr double kWeightSum = 1.0 + 4.0 * (
    kFilter[0] + kFilter[1] + kFilter[2] + kFilter[4] + 2.0 * kFilter[3]);
  ...
  status = LaunchCudaSymmetric5Convolution(
    ...
    kNormalize * kFilter[2], kNormalize * kFilter[1],
    kNormalize * kFilter[4], kNormalize * kFilter[3], stream);
  if (status != cudaSuccess) return status;
  ...
  for (unsigned int deviation = 0; deviation < 2; ++deviation) {
    SelectionInitializeKernel<<<1, 256, 0, stream>>>(
  ...
  const unsigned int scaled_quant_dc = static_cast<unsigned int>(
    static_cast<int>(static_cast<double>(quant_dc) * 4096.0) * 1.6);
  FinalizeQuantizerKernel<<<1, 1, 0, stream>>>(
    statistics, quantizer_params, error, scaled_quant_dc, quant_dc);
  if ((status = CheckLaunch()) != cudaSuccess) return status;
  RawQuantKernel<<<grid, kThreads, 0, stream>>>(
    quant_field, quantizer_params, raw_quant, error, block_count);
  return CheckLaunch();
}
...
cudaError_t LaunchCudaAqEncodeFrame(
  ...
  if (status != cudaSuccess) return status;
  const unsigned int tile_count = geometry.tile_width * geometry.tile_height;
  InitialCflKernel<<<(tile_count + kThreads - 1) / kThreads, kThreads, 0, stream>>>(
    coding_x, coding_y, coding_b, y_to_x, y_to_b, error, geometry);
  if ((status = CheckLaunch()) != cudaSuccess) return status;
  const size_t pixel_count = static_cast<size_t>(geometry.width) * geometry.height;
  const size_t gathered_count = 3 * pixel_count;
  GatherDct8Kernel<<<
    static_cast<unsigned int>((gathered_count + kThreads - 1) / kThreads),
    kThreads, 0, stream>>>(
      transform_x, transform_y, transform_b, gathered, geometry);
  if ((status = CheckLaunch()) != cudaSuccess) return status;
  const unsigned int block_count = geometry.block_width * geometry.block_height;
  status = LaunchCudaDct(
    true, gathered, forward_coefficients, 3 * block_count, 8, 8, stream);
  if (status != cudaSuccess) return status;
  AdjustQuantKernel<<<(block_count + kThreads - 1) / kThreads, kThreads, 0, stream>>>(
    forward_coefficients, quant_tables, raw_quant, quantizer_params,
    y_thresholds, error, block_count, x_matrix_multiplier, b_matrix_multiplier);
  if ((status = CheckLaunch()) != cudaSuccess) return status;
  EncodeFrameKernel<<<block_count, 64, 0, stream>>>(
    forward_coefficients, quant_tables, raw_quant, quantizer_params,
    y_to_x, y_to_b, y_thresholds, quantized_ac, quantized_dc, error,
    geometry, x_matrix_multiplier, b_matrix_multiplier);
  return CheckLaunch();
}
