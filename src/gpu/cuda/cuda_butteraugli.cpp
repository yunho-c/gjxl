...
Status ComputeCudaButteraugliStoragePlan(
    Extent2D source, CudaButteraugliStoragePlan* out) {
  if (out == nullptr || source.empty() ||
      source.width > std::numeric_limits<uint32_t>::max() ||
      source.height > std::numeric_limits<uint32_t>::max()) {
    return Status::InvalidArgument("CUDA Butteraugli plan geometry is invalid");
  }
  CudaButteraugliStoragePlan p;
  p.working_extent = {std::max<size_t>(8, source.width),
                       std::max<size_t>(8, source.height)};
  const bool multiscale = source.width >= 15 && source.height >= 15;
  if (multiscale) p.sub_extent = {(source.width + 1) / 2, (source.height + 1) / 2};
  size_t working_area = 0, requested_area = 0, sub_area = 0;
  if (!source.try_area(&requested_area) || !p.working_extent.try_area(&working_area) ||
      ...
      (multiscale && !p.sub_extent.try_area(&sub_area))) {
    return Status::InvalidArgument("CUDA Butteraugli plan area overflows");
  }
  const size_t partial_count = (requested_area + kReductionWidth - 1) / kReductionWidth;
  ...
  p.cached_reference_bytes =
    (kCudaButteraugliPsychoPlaneCount + 1) * plane_bytes +
    (multiscale ? kCudaButteraugliPsychoPlaneCount * sub_plane_bytes : 0);
  p.gaussian_kernel_bytes = 0;
  for (size_t kernel_size : kKernelSizes) {
    p.gaussian_kernel_bytes += kernel_size * sizeof(float);
  }
  p.peak_comparison_scratch_bytes = (kCudaButteraugliWorkingPlaneCount -
                    (kCudaButteraugliPsychoPlaneCount + 1)) *
                     plane_bytes +
                   2 * partial_count * sizeof(float);

  p.capacity_bytes = capacity;
  if (!p.host.AddVector<float>(33, resource_budget_internal::VectorCapacityPolicy::kFreshExact))
    return Status::OutOfMemory("CUDA Gaussian host storage overflows");
  p.host.retained_bytes = 0;
  *out = p;
  return Status::Ok();
}
...
class CudaPreparedDeviceButteraugli final : public PreparedDeviceButteraugli {
 public:
  ...
  [[nodiscard]] Status PrepareStorage() {
    ...
    CudaButteraugliStoragePlan storage;
    Status status = ComputeCudaButteraugliStoragePlan(requested, &storage);
    if (!status.ok()) return status;
    cached_reference_bytes_ = storage.cached_reference_bytes;
    gaussian_kernel_bytes_ = storage.gaussian_kernel_bytes;
    peak_comparison_scratch_bytes_ = storage.peak_comparison_scratch_bytes;
    status = scratch_.Prepare(backend_, storage.capacity_bytes);
    if (!status.ok()) return status;
