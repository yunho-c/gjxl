...
template <typename T>
void CopyPlaneToContiguous(PlaneView<const T> source, ManagedVector<T>* out) {
  size_t count = 0;
...
template <typename T>
void CopyContiguousToPlane(const ManagedVector<T>& source, PlaneView<T> out) {
...
class CudaPreparedAqEvaluation final
 ...
 public:
  CudaPreparedAqEvaluation() = default;
  static Status ComputeStoragePlan(Extent2D source, const AqEvaluationOptions& evaluation,
                                  bool borrowed_input, CudaCompatibilityStoragePlan* out);
  ...
  Status Prepare(const AqEvaluationPreparation& preparation) {
    ...
    if (preparation.strategies == nullptr ||
        ...
        !IsValidDcQuantization({preparation.options.dc_quantization,
                               preparation.options.dc_prediction,
                               preparation.options.profile.extra_dc_precision}) ||
        (preparation.options.dc_quantization == DcQuantizationMode::kPredictionAware &&
         preparation.options.profile.extra_dc_precision == 0)) {
    ...
    dc_quantization_ = preparation.options.dc_quantization;
    dc_prediction_ = preparation.options.dc_prediction;
  ...
  Status Reconfigure(const AcStrategyGrid& strategies,
    ...
    ManagedVector<uint8_t> candidate;
 ...
 private:
  Status ComputeInitialQuantizationImpl(
    ...
    if (options.uniform) {
      const float q = 0.79f / options.butteraugli_target * options.rescale;
      if (!std::isfinite(q) || q <= 0.0f || !(1.0f / (q + 0.001f) > 0.0f))
        return Status::InvalidArgument("CUDA uniform initial field is invalid");
    }
    const resource_budget_internal::ManagedHostScope resources(
        resource_budget_internal::ResourceClass::kAqScratch);
    if (output != nullptr) {
    ...
    const auto positive = [](const ManagedVector<float>& values) {
 ...
 private:
  ...
  static cudaError_t EncodeInitial(CudaBackend& backend, const void* context) {
    ...
    if (self.initial_options_.uniform) {
      cudaError_t status = LaunchCudaAqUniformInitialField(
          Pointer<float>(self.quant_field_), Pointer<float>(self.strategy_mask_),
          Pointer<float>(self.pixel_mask_), Pointer<unsigned int>(self.error_), self.geometry_,
          0.79f / self.initial_options_.butteraugli_target * self.initial_options_.rescale,
          backend.state_->stream);
      if (status != cudaSuccess) return status;
      return LaunchCudaAqSelectResidentQuantizer(
          Pointer<const float>(self.quant_field_), static_cast<uint32_t>(self.block_count_),
          Pointer<unsigned int>(self.selection_state_),
          Pointer<unsigned int>(self.histogram_), Pointer<float>(self.statistics_),
          Pointer<unsigned int>(self.quantizer_params_), Pointer<int>(self.raw_quant_initial_),
          Pointer<unsigned int>(self.error_), self.quant_dc_, backend.state_->stream);
    }
  ...
  static cudaError_t EncodeFrameSubmission(CudaBackend& backend,
    ...
    return LaunchCudaAqEncodeFrame(self.CodingPointer(0),
      ...
      {self.geometry_.block_width, self.geometry_.block_height, 0, 0,
       static_cast<uint32_t>(self.dc_quantization_), static_cast<uint32_t>(self.dc_prediction_),
       self.profile_.extra_dc_precision, 1},
      self.dc_quantization_ == DcQuantizationMode::kPredictionAware ||
          self.profile_.extra_dc_precision != 0 || self.profile_.adaptive_dc_smoothing,
      backend.state_->stream);
  }
  ...
  DcQuantizationMode dc_quantization_ = DcQuantizationMode::kRound;
  VarDctDcPrediction dc_prediction_ = VarDctDcPrediction::kGradient;
  ...
  std::array<ManagedVector<float>, 3> coding_host_;
  ManagedVector<uint8_t> epf_sharpness_;
  ManagedVector<float> quant_field_host_;
  ManagedVector<float> strategy_mask_host_;
  ManagedVector<float> pixel_mask_host_;
  ManagedVector<int32_t> raw_quant_host_;
  ManagedVector<int8_t> y_to_x_host_;
  ManagedVector<int8_t> y_to_b_host_;
  ManagedVector<int32_t> quantized_ac_host_;
  ManagedVector<int32_t> quantized_dc_host_;
  ManagedVector<vardct_frame_internal::QuantizedAcTransformLayout> transforms_;
...
Status CudaPreparedAqEvaluation::ComputeStoragePlan(
    Extent2D source, const AqEvaluationOptions& evaluation, bool borrowed_input,
    CudaCompatibilityStoragePlan* out) {
  if (out == nullptr || !evaluation.profile.valid() ||
      !IsValidDcQuantization({evaluation.dc_quantization, evaluation.dc_prediction,
                              evaluation.profile.extra_dc_precision}) ||
      (evaluation.dc_quantization == DcQuantizationMode::kPredictionAware &&
       evaluation.profile.extra_dc_precision == 0))
    return Status::InvalidArgument("CUDA frame-only storage options are invalid");
  FrameGeometry geometry;
  Status status = FrameGeometry::Create(source, &geometry);
  if (!status.ok()) return status;
  CudaPreparedAqEvaluation shape;
  shape.pixel_extent_ = geometry.padded_frame();
  shape.block_extent_ = geometry.block_grid().blocks;
  shape.tile_extent_ = ColorTileExtent(shape.pixel_extent_);
  if (!shape.pixel_extent_.try_area(&shape.pixel_count_) ||
      !shape.block_extent_.try_area(&shape.block_count_) ||
      !shape.tile_extent_.try_area(&shape.tile_count_) ||
      shape.pixel_count_ > std::numeric_limits<uint32_t>::max() / 3)
    return Status::InvalidArgument("CUDA frame-only storage exceeds coefficient indexing limits");
  shape.borrowed_input_ = borrowed_input;
  shape.inverse_gaborish_ = evaluation.profile.loop_filter.gaborish;
  CudaCompatibilityStoragePlan plan;
  status = shape.PlanArenas(&plan.persistent_bytes, &plan.staging_bytes);
  if (!status.ok()) return status;
  using enum resource_budget_internal::VectorCapacityPolicy;
  const size_t blocks = shape.block_count_, pixels = shape.pixel_count_;
  if (!plan.host.AddVector<uint8_t>(blocks, kFreshExact, 4) ||
      !plan.host.AddVector<int32_t>(blocks, kFreshExact, 4) ||
      !plan.host.AddVector<int8_t>(shape.tile_count_, kFreshExact, 2) ||
      !plan.host.AddVector<int32_t>(pixels, kFreshExact, 3) ||
      !plan.host.AddVector<vardct_frame_internal::QuantizedAcTransformLayout>(blocks, kFreshExact) ||
      !plan.host.AddVector<float>(blocks, kFreshExact, 2) ||
      !plan.host.AddVector<float>(pixels, kFreshExact, borrowed_input ? 1 : 4))
    return Status::OutOfMemory("CUDA frame-only host storage overflows");
  *out = plan;
  return Status::Ok();
}

Status ComputeCudaFrameOnlyStoragePlan(Extent2D source, const AqEvaluationOptions& evaluation,
    bool borrowed_input, CudaCompatibilityStoragePlan* out) {
  return CudaPreparedAqEvaluation::ComputeStoragePlan(source, evaluation, borrowed_input, out);
}
...
Status CudaBackend::PrepareAqEvaluation(
  ...
  if (preparation.defer_final_transform_metadata &&
    ...
    return Status::InvalidArgument(
        "Deferred CUDA transform metadata requires resident AC search and AQ");
  }
