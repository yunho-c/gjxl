...
void CopyPlaneToContiguous(PlaneView<const T> source, std::vector<T>* out) {
  ...
  for (size_t y = 0; y < source.extent.height; ++y) {
    std::copy_n(
      source.Row(y), source.extent.width,
...
void CopyContiguousToPlane(const std::vector<T>& source, PlaneView<T> out) {
  for (size_t y = 0; y < out.extent.height; ++y) {
    std::copy_n(
      source.data() + y * out.extent.width, out.extent.width, out.Row(y));
  }
}

bool ValidInitialOutput(
  InitialQuantFieldOutput output,
  Extent2D blocks,
  Extent2D pixels) noexcept {
  return output.quant_field.valid() && output.quant_field.extent == blocks &&
    output.strategy_mask.valid() && output.strategy_mask.extent == blocks &&
    output.pixel_mask.valid() && output.pixel_mask.extent == pixels;
}
...
public:
  explicit CudaPreparedAqEvaluation(CudaBackend& backend) : backend_(&backend) {}
  ...
  Status Prepare(const AqEvaluationPreparation& preparation) {
    if (!preparation.frame_only ||
      ...
      return Status::Unavailable(
        "CUDA currently supports only the DCT8 maximum-throughput AQ preparation");
    }
    ...
    Status status = ValidateFiniteImage(
    ...
    status = FrameGeometry::Create(
      preparation.original_linear_rgb.extent(), &geometry);
    if (!status.ok()) return status;
    ...
    if (geometry.padded_frame() != pixels ||
      ...
      return Status::InvalidArgument("CUDA AQ preparation geometry is inconsistent");
    }
    size_t anchors = 0;
    status = preparation.strategies->ForEachAnchor(
    ...
    if (!pixels.try_area(&pixel_count) || !blocks.try_area(&block_count) ||
        ...
        pixel_count > std::numeric_limits<size_t>::max() / (3 * sizeof(float)) ||
        3 * block_count > backend_->state_->maximum_grid_x ||
        (3 * pixel_count + 255) / 256 > backend_->state_->maximum_grid_x) {
      return Status::InvalidArgument("CUDA AQ frame dimensions exceed backend limits");
    }
    ...
    for (size_t channel = 0; channel < 3; ++channel) {
      CopyPlaneToContiguous(preparation.coding_opsin.plane[channel], &coding_host_[channel]);
    }
    ...
    for (size_t channel = 0; channel < 3; ++channel) {
      ...
      status = backend_->CopyHostToDevice(
        *coding_[channel], coding_host_[channel].data(),
    ...
    if (!status.ok()) return status;
    status = backend_->CopyHostToDevice(
      *epf_device_, epf_sharpness_.data(), block_count_, 0);
    if (!status.ok()) return status;

    std::array<float, 384> quant_tables{};
    std::copy(
      quantization_internal::kDct8Dequant.begin(),
      quantization_internal::kDct8Dequant.end(), quant_tables.begin());
    std::copy(
      quantization_internal::kDct8InverseDequant.begin(),
    ...
    if (!status.ok()) return status;
    status = backend_->CopyHostToDevice(
      *quant_tables_, quant_tables.data(), sizeof(quant_tables), 0);
    if (!status.ok()) return status;
  ...
  Status Reconfigure(
    ...
    if (!strategies.complete() || strategies.extent() != block_extent_ ||
      ...
      return Status::InvalidArgument("CUDA AQ reconfiguration geometry is invalid");
    }
    size_t count = 0;
    ...
    status = backend_->CopyHostToDevice(
      *epf_device_, candidate.data(), candidate.size(), 0);
    if (!status.ok()) return status;
  ...
  Status ComputeInitialQuantization(
    ...
    if (!ValidInitialOutput(output, block_extent_, pixel_extent_) ||
        ...
        quantizer == nullptr || !std::isfinite(quant_dc) || quant_dc <= 0.0f ||
    ...
    if (submission == nullptr) {
      return Status::Internal("CUDA initial quantization returned no submission");
    }
    ...
    if (status.ok()) status = Read(*quantizer_params_, params.data(), params.size());
    if (status.ok()) status = Read(
      *quant_field_, quant_field_host_.data(), quant_field_host_.size());
    if (status.ok()) status = Read(
      *strategy_mask_, strategy_mask_host_.data(), strategy_mask_host_.size());
    if (status.ok()) status = Read(
    ...
    if (!status.ok() || !positive(quant_field_host_) ||
      ...
      initial_ready_ = false;
      return Status::DeviceError("CUDA initial quantization readback is invalid");
    }
  ...
  Status EncodeFrame(
    ...
    if (input.quantizer.global_scale != quantizer_.global_scale ||
      ...
      return Status::InvalidArgument(
        "CUDA EncodeFrame quantizer does not match resident initial quantization");
    }
    ...
    status = Read(*error_, &device_error, 1);
    if (status.ok()) status = Read(
      *raw_quant_work_, raw_quant_host_.data(), raw_quant_host_.size());
    ...
    if (status.ok()) status = Read(
      *quantized_ac_, quantized_ac_host_.data(), quantized_ac_host_.size());
    if (status.ok()) status = Read(
      *quantized_dc_, quantized_dc_host_.data(), quantized_dc_host_.size());
    if (!status.ok()) return status;
    if (device_error != 0 ||
        std::any_of(raw_quant_host_.begin(), raw_quant_host_.end(),
    ...
    quantized_dc.plane[0] = {
      quantized_dc_host_.data(), block_extent_, block_extent_.width};
    quantized_dc.plane[1] = {
      quantized_dc_host_.data() + block_count_,
      block_extent_, block_extent_.width};
    quantized_dc.plane[2] = {
      quantized_dc_host_.data() + 2 * block_count_,
      block_extent_, block_extent_.width};
...
private:
  ...
  Status Allocate(
    ...
    if (status.ok()) {
      if (persistent) persistent_bytes_ += bytes;
      else staging_bytes_ += bytes;
    }
    return status;
  }
  ...
  static cudaError_t EncodeInitial(CudaBackend& backend, const void* context) {
    auto& self = *static_cast<CudaPreparedAqEvaluation*>(
  ...
  static cudaError_t EncodeFrameSubmission(
    CudaBackend& backend,
    const void* context) {
    auto& self = *static_cast<CudaPreparedAqEvaluation*>(
      const_cast<void*>(context));
    cudaError_t status = cudaMemcpyAsync(
      Buffer(self.raw_quant_work_)->pointer(),
      Buffer(self.raw_quant_initial_)->pointer(),
      self.block_count_ * sizeof(int32_t), cudaMemcpyDeviceToDevice,
      backend.state_->stream);
    if (status != cudaSuccess) return status;
    std::array<const float*, 3> transform{};
    for (size_t channel = 0; channel < 3; ++channel) {
      const auto* coding = static_cast<const float*>(
        Buffer(self.coding_[channel])->pointer());
      ...
      if (self.inverse_gaborish_) {
        ...
        auto* filtered = static_cast<float*>(
          Buffer(self.filtered_[channel])->pointer());
        status = LaunchCudaSymmetric5Convolution(
          coding, filtered, self.geometry_.width, self.geometry_.height,
          self.geometry_.width, self.geometry_.width,
          weights.distance0, weights.distance1, weights.distance2,
          weights.distance4, weights.distance8, weights.distance5,
          backend.state_->stream);
        if (status != cudaSuccess) return status;
    ...
    return LaunchCudaAqEncodeFrame(
      ...
      static_cast<const unsigned int*>(Buffer(self.quantizer_params_)->pointer()),
...
Status CudaBackend::PrepareAqEvaluation(
  ...
  try {
    ...
    *prepared = std::move(result);
    ...
    return Status::OutOfMemory("Unable to allocate CUDA prepared AQ evaluation");
