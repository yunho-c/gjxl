...
Status CudaBackend::EvaluateAcStrategyCandidateBatchesImpl(
  ...
  // The encoder submits one batch for each of its seven strategy families.
  // Keep that fixed-size validation metadata on the stack; direct backend
  // callers can still submit longer sequences using accounted storage.
  std::array<ValidatedAcStrategyBatch, 7> inline_batches{};
  ManagedVector<ValidatedAcStrategyBatch> overflow_batches;
  std::span<ValidatedAcStrategyBatch> validated_batches = inline_batches;
  size_t validated_count = 0;
  try {
    if (batches.size() > inline_batches.size()) {
      overflow_batches.resize(batches.size());
      validated_batches = overflow_batches;
    }
    for (const AcStrategyCandidateBatch& batch : batches) {
      ...
      if (!status.ok()) return status;
      if (batch.candidate_count != 0) {
        validated_batches[validated_count++] = validated;
      }
    }
  ...
  if (validated_count == 0) return Status::Ok();
