...
static_assert(sizeof(CudaAqResidentParams) == 76);
...
template <typename T>
bool HostPlaneSpecified(PlaneView<T> plane) noexcept {
...
Status ValidateOptions(const AqEvaluationOptions& options) {
  if (!IsValidDcQuantization({options.dc_quantization, options.dc_prediction,
    ...
    return Status::InvalidArgument("CUDA resident AQ DC quantization is invalid");
  }
...
Status PackQuantTables(ManagedVector<float>* packed) {
  try {
    packed->clear();
...
class CudaPreparedResidentAqEvaluation final
 ...
 public:
  CudaPreparedResidentAqEvaluation() = default;
  static Status ComputeStoragePlan(const CudaResidentStorageOptions& options,
  ...
  Status ComputeInitialQuantizationForEncodingProfiled(
    ...
    CudaProfileCapture capture(*backend_, "aq.initial_quantization", mode);
  ...
  Status PrepareResidentEncodingPolicyProfiled(
    ...
    Status status = ValidateCudaProfileRequest(mode, profile);
    if (!status.ok()) return status;
    CudaProfileCapture capture(*backend_, "aq.prepare_encoding_policy", mode);
  ...
  Status AdjustQuantFieldResidentProfiled(
    ...
    Status status = ValidateCudaProfileRequest(mode, profile);
    if (!status.ok()) return status;
    CudaProfileCapture capture(*backend_, "aq.adjust_quant_field", mode);
  ...
  Status EvaluateResidentButteraugliPolicyProfiled(
    ...
    Status status = ValidateCudaProfileRequest(mode, profile);
    if (!status.ok()) return status;
    CudaProfileCapture capture(*backend_, "aq.resident_policy", mode);
  ...
  Status Prepare(const AqEvaluationPreparation& preparation) {
    ...
    if (preparation.options.evaluation_free &&
      ...
      return Status::InvalidArgument(
          "Evaluation-free CUDA preparation requires Butteraugli encoding");
    }
    if (preparation.omit_initial_search_data && !resident_frontend) {
      return Status::InvalidArgument(
          "Omitting CUDA search data requires the complete resident frontend");
    }
    ...
    borrowed_input_ = resident_input;
    ...
    if (options_.evaluation_free) filter_xyb_stage_count_ = 0;

    Metadata metadata;
    status = DeferInitialMetadata(*preparation.strategies,
    ...
    if (!status.ok()) return status;
    try {
      if (!options_.evaluation_free) block_readback_.resize(block_count_);
      if (options_.metric == AqEvaluationMetric::kMaximumError)
      ...
      for (size_t channel = 0; channel < 3; ++channel) {
        if (!options_.evaluation_free) {
          status = UploadPlane(*backend_,
            preparation.original_linear_rgb.plane[channel], original_[channel]);
        }
        if (status.ok()) {
          status = UploadPlane(*backend_,
  ...
  Status ComputeInitialQuantization(
    ...
    if (!FinitePositive(options.butteraugli_target) ||
        ...
        (omit_initial_search_data_ &&
         (HostPlaneSpecified(output.strategy_mask) ||
          HostPlaneSpecified(output.pixel_mask) ||
  ...
  Status PrepareResidentEncodingPolicy(
    ...
    if (nonlinear_iterations > 20)
      return Status::InvalidArgument("CUDA resident final CfL iteration limit is invalid");
    ...
    nonlinear_cfl_iterations_ = nonlinear_iterations;
  ...
  Status GetResidentAcStrategyInputs(
    ...
    const ConstDeviceImage3View search_opsin =
        options_.profile.loop_filter.gaborish ? ConstImage(reconstructed_) :
        (has_borrowed_input() ? borrowed_coding_ : ConstImage(coding_));
    *inputs = {.opsin = search_opsin,
  ...
  Status PrepareInvariantColorCorrelationResident(ConstPlaneF32View quant_field,
    ...
    if (final_transform_metadata_pending_)
      return Status::FailedPrecondition(
          "CUDA resident AQ requires final transform reconfiguration");
    invariant_color_correlation_ready_ = false;
    resident_encoding_policy_ready_ = false;
    forward_coefficients_ready_ = false;
    color_correlation_pending_ = false;
  ...
  Status Evaluate(AqEvaluationInput input, AqEvaluationOutput output) override {
    ...
    if (options_.evaluation_free) {
      return Status::FailedPrecondition(
          "Evaluation-free CUDA preparation cannot evaluate a field");
    }
  ...
  Status EvaluateResidentButteraugliPolicy(
    ...
    const bool frame_requested = output.frame != nullptr || output.completed_frame != nullptr;
    ...
    if ((output.frame != nullptr && output.completed_frame != nullptr) ||
        input.iterations > 4 || !FinitePositive(input.butteraugli_target) ||
        ...
        (!input.evaluate_final_field &&
         (!frame_requested || block_requested ||
          reconstruction_requested))) {
      return Status::InvalidArgument(
    ...
    status = resource_budget_internal::PublicationVector<double>::Create(
        score_count, &candidate_scores,
        resource_budget_internal::ResourceClass::kAqScratch);
    if (!status.ok()) return status;

    std::unique_ptr<CudaCompletedVarDctFrame> candidate_completed;
    if (output.completed_frame != nullptr) {
      try {
        candidate_completed = std::make_unique<CudaCompletedVarDctFrame>();
      } catch (const resource_budget_internal::ManagedAllocationFailure& failure) {
        return failure.status();
      } catch (const std::bad_alloc&) {
        return Status::OutOfMemory("Unable to allocate CUDA completed frame owner");
      }
    }
    std::unique_lock lock(mutex_, std::try_to_lock);
    if (!lock.owns_lock()) {
    ...
    if (!invariant_color_correlation_ready_ ||
        (score_count != 0 && butteraugli_ == nullptr)) {
      return Status::FailedPrecondition(
...
#if defined(GJXL_CUDA_COMPACT_AC)
    compact_active_ =
        frame_requested && coefficient_count_ >= 8 &&
    ...
    PolicyContext context{this, input, score_count,
                          !forward_coefficients_ready_,
                          color_correlation_pending_, frame_requested};
    std::unique_ptr<GpuSubmission> submission;
    status = backend_->SubmitCompute(
    ...
    size_t policy_readback_count = 0;
    if (score_count != 0) {
    ...
    const bool early_population = ReadPopulationEarly(frame_requested);
    if (early_population) {
    ...
    if (compact_flags_ > 3 || compact_flags_ == 2) {
      return Invalidate(
          Status::DeviceError("CUDA compact AC width flags are invalid"));
    }
    for (size_t index = 0; index < score_count; ++index) {
      ...
      candidate_scores.mutable_view()[index] = score;
    }
    if (quant_requested &&
    ...
    if (candidate_completed != nullptr) {
      candidate_completed->frame = std::move(candidate_frame);
      *output.completed_frame = std::move(candidate_completed);
    } else if (output.frame != nullptr) {
      *output.frame = std::move(candidate_frame);
    }
    forward_coefficients_ready_ = true;
    color_correlation_pending_ = false;
    return Status::Ok();
 ...
 private:
  ...
  struct Metadata {
    ...
    ManagedVector<CudaAqAnchor> device_anchors;
    ManagedVector<uint64_t> packing_offsets;
    ManagedVector<uint8_t> sampled_dct8;
    uint16_t population_mask = 0;
    ManagedVector<HostAnchor> row_major_anchors;
    ManagedVector<uint8_t> epf_sharpness;
    ManagedVector<CudaAqColorTransformRecord> color_transforms;
    ManagedVector<uint32_t> color_tile_offsets;
    ManagedVector<vardct_frame_internal::QuantizedAcTransformLayout> layouts;
  };
  ...
  Status ComputeInitialQuantizationImpl(
    ...
    if (options.uniform &&
        (!FinitePositive(uniform_quant) ||
         !FinitePositive(1.0f / (uniform_quant + 0.001f)))) {
      return Status::InvalidArgument("CUDA uniform initial quantization is invalid");
    }
    if (output != nullptr && !omit_initial_search_data_) {
      try {
    ...
    size_t initial_readback_count = 1;
    ...
    for (size_t y = 0; !omit_initial_search_data_ && y < coding_extent_.height; ++y) {
  ...
  Status DeferInitialMetadata(const AcStrategyGrid &strategies,
    ...
    // An explicit deferred request also permits provisional mixed grids, but
    // their layouts cannot be used until the caller successfully reconfigures.
    if (!resident_frontend_ ||
        coefficient_count_ > std::numeric_limits<uint32_t>::max() ||
        3 * block_count_ > backend_->state_->maximum_grid_x ||
        coefficient_count_ / 256 + (coefficient_count_ % 256 != 0) >
            backend_->state_->maximum_grid_x) {
      return final_transform_metadata_pending_
          ? Status::InvalidArgument("Deferred CUDA transform geometry exceeds device limits")
          : Status::Ok();
    }
    bool all_dct8 = true;
    size_t anchors = 0;
    Status status =
        strategies.ForEachAnchor([&](size_t x, size_t y, AcStrategyType strategy) {
          if (StrategyBatchIndex(strategy) >= kSupportedStrategies.size() ||
              !chroma_from_luma_internal::StrategyFitsColorTile(x, y, strategy))
            return Status::InvalidArgument("CUDA resident AQ provisional strategy is unsupported");
    ...
    if (!status.ok())
      return status;
    if (!final_transform_metadata_pending_ && (!all_dct8 || anchors != block_count_))
      return Status::Ok();
  ...
  Status BuildMetadata(const AcStrategyGrid& strategies,
    ...
    try {
      ...
      ManagedVector<size_t> group_used(group_packed_offsets_.size(), 0);
      ...
      ManagedVector<uint32_t> positions = candidate.color_tile_offsets;
      ManagedVector<uint32_t> tile_value_offsets(tile_count_, 0);
      for (const HostAnchor& anchor : candidate.row_major_anchors) {
        const size_t tile =
            (anchor.block_y / 8) * tile_extent_.width + anchor.block_x / 8;
        const CudaAqExactBatch& batch = candidate.batches[anchor.batch_index];
  ...
  Status PlanArenas(size_t* persistent_out = nullptr, size_t* staging_out = nullptr) {
    ...
    if (!has_borrowed_input() && !options_.evaluation_free) {
    ...
    const size_t coding_image_count = static_cast<size_t>(!has_borrowed_input()) +
        static_cast<size_t>(NeedsReconstructedOpsin());
    ...
    for (size_t channel = 0; !options_.evaluation_free &&
         channel < 3 && status.ok(); ++channel) {
    ...
    if (!options_.evaluation_free &&
        options_.metric == AqEvaluationMetric::kButteraugli && status.ok()) {
      status = plan_staging(DeviceElementType::kF32, block_extent_,
                            block_extent_.width);
      if (status.ok()) {
    ...
    if (!status.ok()) return status;
    if (persistent_out != nullptr && staging_out != nullptr) {
      *persistent_out = persistent_bytes;
      *staging_out = staging_bytes;
      return Status::Ok();
    }
  ...
  Status AllocateArenas() {
    ...
    for (DevicePlaneView& plane : reconstructed_) {
      if (!NeedsReconstructedOpsin()) break;
    ...
    for (DevicePlaneView& plane : reconstructed_linear_) {
      if (options_.evaluation_free) break;
      status = AllocatePlane(persistent_, DeviceElementType::kF32,
  ...
  Status PrepareReconstructionReadback() {
    ...
    try {
      std::array<ManagedVector<float>, 3> candidate;
  ...
  Status AssembleFrame(const Quantizer& quantizer, VarDctEncoderFrame* frame) {
    ...
    if (compact_active_ && (compact_flags_ & 2u) == 0)
      return AssembleTypedFrame(quantizer, frame, compact_readback_i16_);
    return AssembleTypedFrame(quantizer, frame, quantized_readback_);
  }
  ...
  Status AssembleTypedFrame(const Quantizer &quantizer,
    ...
    const DevicePlaneView source = sizeof(T) == 4
                                       ? reconstruction_coefficients_device_
                                       : quantized_device_;
    const size_t payload_offset = sizeof(T) == 2 ? coefficient_count_ : 0;
    ManagedVector<CudaDeviceToHostCopy> readbacks;
    try {
      ...
      for (const AcReadbackRun& run : ac_readback_runs_) {
        T *destination = owner.data() + run.group_offset;
        ...
        readbacks.push_back({source.buffer, destination,
                             run.row_values * sizeof(T),
                             source.offset_bytes + payload_offset +
                                 run.packed_offset * sizeof(T),
                             run.rows, run.row_values * sizeof(T),
                             kVarDctAcGroupCoefficientCapacity * sizeof(T)});
      }
    ...
    return vardct_frame_internal::AssembleVarDctEncoderFrame(
        vardct_frame_internal::QuantizedFrameAssemblyInputT<T>{
            ...
            .raw_quant_field = {raw_readback_.data(), block_extent_,
                                block_extent_.width},
            .quantizer = &quantizer,
            .y_to_x = {y_to_x_readback_.data(), tile_extent_,
                       tile_extent_.width},
            .y_to_b = {y_to_b_readback_.data(), tile_extent_,
                       tile_extent_.width},
  ...
  static cudaError_t EncodeInitialQuantization(CudaBackend& backend,
    ...
    float* strategy_mask = self.omit_initial_search_data_ ? nullptr :
        Pointer<float>(self.initial_strategy_device_);
    float* pixel_mask = self.omit_initial_search_data_ ? nullptr :
        Pointer<float>(self.initial_pixel_device_);
    cudaError_t status = context.options.uniform
        ? LaunchCudaAqUniformInitialField(
            Pointer<float>(self.quant_field_device_), strategy_mask, pixel_mask,
            Pointer<unsigned int>(self.error_device_), geometry,
            0.79f / context.options.butteraugli_target * context.options.rescale,
            backend.state_->stream)
        : LaunchCudaAqInitialField(self.CodingPointer(0),
  ...
  static cudaError_t EncodeFinalColorCorrelation(
      CudaBackend& backend, CudaPreparedResidentAqEvaluation& self) {
    if (self.nonlinear_cfl_iterations_ != 0) {
    return LaunchCudaAqFinalColorCorrelationNonlinear(
        ...
        Pointer<signed char>(self.y_to_b_device_),
        Pointer<unsigned int>(self.error_device_),
        static_cast<uint32_t>(self.tile_count_), self.nonlinear_cfl_iterations_, backend.state_->stream);
    }
  ...
  static cudaError_t EncodeReconstruction(CudaBackend& backend,
    ...
    if (self.DeferredDc()) {
      // DC groups and smoothing neighborhoods span transform batches. Finish
      // all samples before quantization, then rebuild LLF before any inverse.
      status = EncodeDcProcessing(backend, self, true);
      if (status != cudaSuccess) return status;
      for (const CudaAqExactBatch& batch : self.batches_) {
        if (batch.anchor_count == 0) continue;
        status = LaunchCudaAqDcLowFrequencies(
            Pointer<const CudaAqAnchor>(self.anchors_device_),
            Pointer<const float>(self.NeedsDcSmoothing()
                ? self.smoothed_dc_device_ : self.dc_device_),
            Pointer<float>(self.reconstruction_coefficients_device_), batch,
            static_cast<uint32_t>(self.block_extent_.width),
  ...
  static cudaError_t EncodePackedAc(
    ...
    if (self.compact_active_) {
      // Quantized source batches and population consumers are finished. Reuse
      // their allocation for byte[N], word[2N], flags[4], keeping packed int32
      // coefficients intact for overflow fallback. N is 8-aligned and >= 8.
      auto *scratch = Pointer<uint8_t>(self.quantized_device_);
      auto *flags =
          reinterpret_cast<uint32_t *>(scratch + 3 * self.coefficient_count_);
      cudaError_t status =
          cudaMemsetAsync(flags, 0, sizeof(uint32_t), backend.state_->stream);
      if (status != cudaSuccess)
        return status;
      return LaunchCudaCompactAc(
          Pointer<const int32_t>(self.reconstruction_coefficients_device_),
          static_cast<uint32_t>(self.coefficient_count_),
          reinterpret_cast<uint32_t *>(scratch),
          reinterpret_cast<uint32_t *>(scratch + self.coefficient_count_),
          flags, backend.state_->stream);
    }
    return cudaSuccess;
  }
  ...
  static cudaError_t EncodeResidentPolicy(CudaBackend& backend,
    ...
    if (!context.input.evaluate_final_field) {
      ...
      status = EncodeResidentQuantizer(
      ...
      if (self.DeferredDc()) {
        const cudaError_t status = EncodeDcProcessing(backend, self, false);
        if (status != cudaSuccess) return status;
      }
    }
    return context.pack_ac ? EncodePackedAc(backend, self) : cudaSuccess;
  ...
  bool has_borrowed_input() const noexcept {
    return borrowed_input_;
  }

  bool NeedsReconstructedOpsin() const noexcept {
    return !options_.evaluation_free ||
        (resident_frontend_ && options_.profile.loop_filter.gaborish);
  }

  bool NeedsDcSmoothing() const noexcept {
    return !options_.evaluation_free && options_.profile.adaptive_dc_smoothing;
  }

  bool DeferredDc() const noexcept {
    return options_.dc_quantization == DcQuantizationMode::kPredictionAware ||
           options_.profile.extra_dc_precision != 0 || options_.profile.adaptive_dc_smoothing;
  }

  static cudaError_t EncodeDcProcessing(CudaBackend& backend,
      CudaPreparedResidentAqEvaluation& self, bool reconstruction) {
    const CudaDcProcessingParams params{
        static_cast<uint32_t>(self.block_extent_.width),
        static_cast<uint32_t>(self.block_extent_.height), 0, 0,
        static_cast<uint32_t>(self.options_.dc_quantization),
        static_cast<uint32_t>(self.options_.dc_prediction),
        self.options_.profile.extra_dc_precision, 1};
    cudaError_t status = LaunchCudaDcQuantization(
        Pointer<float>(self.dc_device_), Pointer<int>(self.quantized_dc_device_),
        Pointer<unsigned int>(self.error_device_), params,
        Pointer<const unsigned int>(self.quantizer_device_), backend.state_->stream);
    if (status != cudaSuccess || !reconstruction || !self.NeedsDcSmoothing()) return status;
    return LaunchCudaDcSmoothing(Pointer<const float>(self.dc_device_),
        Pointer<float>(self.smoothed_dc_device_), Pointer<unsigned int>(self.error_device_),
        params, Pointer<const unsigned int>(self.quantizer_device_), backend.state_->stream);
  }
  ...
  DevicePlaneView smoothed_dc_device_{};
  ...
  ManagedVector<size_t> group_packed_offsets_;
  ManagedVector<AcReadbackRun> ac_readback_runs_;
  ...
  ManagedVector<uint8_t> epf_sharpness_;
  ManagedVector<vardct_frame_internal::QuantizedAcTransformLayout> layouts_;
  ManagedVector<float> block_readback_;
  ManagedVector<float> maximum_readback_;
  ManagedVector<int32_t> raw_readback_;
  OverwriteArray<int32_t> quantized_readback_;
  OverwriteArray<int8_t> compact_readback_i8_;
  ...
  ManagedVector<int32_t> quantized_dc_readback_;
  ManagedVector<int8_t> y_to_x_readback_;
  ManagedVector<int8_t> y_to_b_readback_;
  ManagedVector<float> quant_field_readback_;
  ManagedVector<float> initial_strategy_readback_;
  ManagedVector<float> initial_pixel_readback_;
  std::array<float, 5> policy_score_readback_{};
  std::array<ManagedVector<float>, 3> linear_readback_;
...
Status CudaPreparedResidentAqEvaluation::ComputeStoragePlan(
  ...
  if (out == nullptr || (o.omit_initial_search_data && !o.resident_frontend) ||
      (o.evaluation.evaluation_free && o.evaluation.metric != AqEvaluationMetric::kButteraugli))
    return Status::InvalidArgument("CUDA resident storage options are invalid");
  Status status = ValidateOptions(o.evaluation);
  if (!status.ok()) return status;
  FrameGeometry geometry;
  status = FrameGeometry::Create(o.source, &geometry);
  if (!status.ok()) return status;
  CudaPreparedResidentAqEvaluation shape;
  shape.source_extent_ = o.source;
  shape.coding_extent_ = geometry.padded_frame();
  shape.block_extent_ = geometry.block_grid().blocks;
  shape.tile_extent_ = shape.coding_extent_.ceil_div(kColorTileDimension);
  if (!shape.source_extent_.try_area(&shape.source_count_) ||
      !shape.coding_extent_.try_area(&shape.coding_count_) ||
      !shape.block_extent_.try_area(&shape.block_count_) ||
      !shape.tile_extent_.try_area(&shape.tile_count_) ||
      shape.coding_count_ > std::numeric_limits<uint32_t>::max() / 3)
    return Status::InvalidArgument("CUDA resident storage exceeds coefficient indexing limits");
  shape.coefficient_count_ = 3 * shape.coding_count_;
  shape.options_ = o.evaluation;
  shape.resident_frontend_ = o.resident_frontend;
  shape.omit_initial_search_data_ = o.omit_initial_search_data;
  shape.borrowed_input_ = o.borrowed_input;
  shape.filter_xyb_stage_count_ = size_t(o.evaluation.profile.loop_filter.gaborish) +
      o.evaluation.profile.loop_filter.epf_options.iterations;
  if (shape.FuseGaborishEpf()) --shape.filter_xyb_stage_count_;
  if (o.evaluation.metric != AqEvaluationMetric::kMaximumError &&
      o.evaluation.profile.loop_filter.epf_options.iterations != 0)
    --shape.filter_xyb_stage_count_;
  if (o.evaluation.evaluation_free) shape.filter_xyb_stage_count_ = 0;
  CudaResidentStoragePlan p;
  status = shape.PlanArenas(&p.persistent_bytes, &p.staging_bytes);
  if (!status.ok()) return status;
  if (!o.evaluation.evaluation_free && o.evaluation.metric == AqEvaluationMetric::kButteraugli) {
    status = ComputeCudaButteraugliStoragePlan(o.source, &p.butteraugli);
    if (!status.ok()) return status;
  }
  using enum resource_budget_internal::VectorCapacityPolicy;
  const size_t blocks = shape.block_count_, tiles = shape.tile_count_;
  size_t groups = 0;
  if (!shape.block_extent_.ceil_div(kVarDctAcGroupBlockDimension).try_area(&groups) ||
      groups > std::numeric_limits<size_t>::max() / (3 * kVarDctAcGroupCoefficientCapacity))
    return Status::OutOfMemory("CUDA resident AC capacity overflows");
  const size_t ac_capacity = groups * 3 * kVarDctAcGroupCoefficientCapacity;
  // Persistent group packing/runs and readback-copy descriptors. Metadata's
  // temporary group-used table may coexist with these persistent owners.
  if (!p.host.AddVector<size_t>(groups, kFreshExact, 2) ||
      !p.host.AddVector<AcReadbackRun>(groups, kGrowing) ||
      !p.host.AddVector<CudaDeviceToHostCopy>(groups + 5, kFreshExact) ||
      // Current and replacement grids, sharpness, row-major anchors/layouts.
      !p.host.AddVector<uint8_t>(blocks, kFreshExact, 4) ||
      !p.host.AddVector<HostAnchor>(blocks, kFreshExact, 2) ||
      !p.host.AddVector<vardct_frame_internal::QuantizedAcTransformLayout>(blocks, kFreshExact, 2) ||
      // Grouped anchor vectors together contain at most one record per block.
      !p.host.AddVector<CudaAqAnchor>(blocks, kGrowing) ||
      !p.host.AddVector<CudaAqAnchor>(blocks, kFreshExact) ||
      !p.host.AddVector<uint64_t>(blocks, kFreshExact) ||
      !p.host.AddVector<uint8_t>(blocks, kFreshExact) ||
      !p.host.AddVector<CudaAqColorTransformRecord>(blocks, kFreshExact) ||
      !p.host.AddVector<uint32_t>(tiles + 1, kFreshExact, 2) ||
      !p.host.AddVector<uint32_t>(tiles, kFreshExact) ||
      !p.host.AddVector<float>(kQuantTableValueCount, kFreshExact) ||
      // Raw quantization, DC readbacks, initial/adjusted quant field and CfL.
      !p.host.AddVector<int32_t>(blocks, kFreshExact, 4) ||
      !p.host.AddVector<float>(blocks, kFreshExact) ||
      !p.host.AddVector<int8_t>(tiles, kFreshExact, 2) ||
      // Published score candidates may coexist with the previous policy output.
      !p.host.AddVector<double>(5, kFreshExact, 2))
    return Status::OutOfMemory("CUDA resident host storage overflows");
  if (!o.evaluation.evaluation_free &&
      (!p.host.AddVector<float>(blocks, kFreshExact) ||
       !p.host.AddVector<float>(shape.source_count_, kFreshExact, 3)))
    return Status::OutOfMemory("CUDA resident diagnostic storage overflows");
  if (o.evaluation.metric == AqEvaluationMetric::kMaximumError &&
      !p.host.AddVector<float>(blocks, kFreshExact, 3))
    return Status::OutOfMemory("CUDA maximum-error storage overflows");
  if (o.resident_frontend && !o.omit_initial_search_data &&
      (!p.host.AddVector<float>(blocks, kFreshExact) ||
       !p.host.AddVector<float>(shape.coding_count_, kFreshExact)))
    return Status::OutOfMemory("CUDA initial diagnostic storage overflows");
  // Compact i8/i16 readbacks are compile-time optional, but bound every native
  // owner so this recipe covers the supported build configurations uniformly.
  if (!p.native_ac.AddVector<int32_t>(ac_capacity, kFreshExact) ||
      !p.native_ac.AddVector<int16_t>(ac_capacity, kFreshExact) ||
      !p.native_ac.AddVector<int8_t>(ac_capacity, kFreshExact))
    return Status::OutOfMemory("CUDA native AC storage overflows");
  const size_t words = shape.coefficient_count_ / 64;
  if (shape.coefficient_count_ >= size_t{3} * 256 * 256) {
    p.sparse_header_bytes = words * (sizeof(uint64_t) + sizeof(uint32_t)) + sizeof(uint32_t);
    if (!p.native_ac.AddVector<uint64_t>(words, kFreshExact) ||
        !p.native_ac.AddVector<uint32_t>(words, kFreshExact) ||
        !p.native_ac.AddVector<int32_t>(shape.coefficient_count_ / 8, kFreshExact))
      return Status::OutOfMemory("CUDA native sparse AC storage overflows");
  }
  *out = p;
  return Status::Ok();
}

Status ComputeCudaResidentStoragePlan(
    const CudaResidentStorageOptions& options, CudaResidentStoragePlan* out) {
  return CudaPreparedResidentAqEvaluation::ComputeStoragePlan(options, out);
}
