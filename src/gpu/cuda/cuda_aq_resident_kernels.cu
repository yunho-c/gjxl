...
__device__ bool FinalCflLayout(
  uint32_t strategy,
  uint32_t& coefficient_width,
  uint32_t& coefficient_height,
  uint32_t& low_frequency_width,
  uint32_t& low_frequency_height) {
...
// Preserve the CPU objective's four accumulation lanes and coefficient order.
// The fast kernel keeps its existing warp layout and shared-memory footprint.
__device__ float NonlinearCflDerivative(float a, float residual) {
  float value = (2.0f / 3.0f) * a * (fabsf(residual) + 1.0f);
  return residual < 0.0f ? -value : value;
}

__global__ void NonlinearFinalCflKernel(
    const CudaAqColorTransformRecord* transforms, const uint32_t* tile_offsets,
    const float* quant_tables, const float* forward_coefficients,
    const int* raw_quant, const unsigned int* resident_quantizer, signed char* y_to_x,
    signed char* y_to_b, unsigned int* error, uint32_t tile_count,
    uint32_t nonlinear_iterations) {
  const uint32_t tile_index = blockIdx.x;
  const uint32_t lane = threadIdx.x;
  if (tile_index >= tile_count) return;
  const uint32_t begin = tile_offsets[tile_index];
  const uint32_t end = tile_offsets[tile_index + 1];
  if (begin >= end) {
    if (lane == 0) atomicOr(error, 64u);
    return;
  }
    const uint32_t global_scale = resident_quantizer[0];
    const CudaAqColorTransformRecord last = transforms[end - 1u];
    const float sample_count = float(last.tile_value_offset + last.coefficient_count);
    __shared__ float estimates[2];
    __shared__ uint32_t done[2];
    __shared__ float derivative_lanes[6][4];
    // Dynamic storage keeps the fast path's threadgroup footprint unchanged.
    __shared__ float chunk_derivatives[6 * 128];
    __shared__ uint32_t chunk_mask[128];
    if (lane == 0u) {
      estimates[0] = 0.0f;
      estimates[1] = 0.0f;
      done[0] = 0u;
    ...
    __syncthreads();
    for (uint32_t iteration = 0u; iteration < nonlinear_iterations; ++iteration) {
      float derivative = 0.0f;
      for (uint32_t transform_index = begin; transform_index < end;
           ++transform_index) {
        const CudaAqColorTransformRecord transform = transforms[transform_index];
        uint32_t coefficient_width = 0u;
        uint32_t coefficient_height = 0u;
        uint32_t low_frequency_width = 0u;
        uint32_t low_frequency_height = 0u;
        if (!FinalCflLayout(
        ...
        const TableOffsets table_offsets = QuantTableOffsets(transform.strategy);
        for (uint32_t chunk = 0u; chunk < transform.coefficient_count; chunk += 128u) {
          const uint32_t coefficient = chunk + lane;
          uint32_t mask = 0u;
          if (coefficient < transform.coefficient_count &&
            ...
            const uint32_t base = transform.coefficient_offset + coefficient;
            const float coefficient_y =
              forward_coefficients[base + transform.channel_stride];
            const float coefficient_x = forward_coefficients[base];
            const float coefficient_b =
            ...
            const float value_y_b = coefficient_y *
              quant_tables[
                table_offsets.inverse_dequant + 2u * transform.coefficient_count + coefficient] *
              quant_scale;
            ...
            if (!isfinite(a[0]) || !isfinite(a[1]) ||
              ...
              for (uint32_t channel = 0u; channel < 2u; ++channel) {
                ...
                const float center = fmaf(a[channel], estimates[channel], b[channel]);
                // Center residual gates all three derivatives.
                if (fabsf(center) >= 100.0f) continue;
                mask |= 1u << channel;
                chunk_derivatives[(3u * channel) * 128u + lane] =
                  NonlinearCflDerivative(a[channel], center);
                chunk_derivatives[(3u * channel + 1u) * 128u + lane] = NonlinearCflDerivative(a[channel],
                  fmaf(a[channel], estimates[channel] + 100.0f, b[channel]));
                chunk_derivatives[(3u * channel + 2u) * 128u + lane] = NonlinearCflDerivative(a[channel],
                  fmaf(a[channel], estimates[channel] - 100.0f, b[channel]));
              }
            }
          }
          ...
          __syncthreads();
          if (lane < 24u) {
            const uint32_t component = lane / 4u;
            const uint32_t first = ((lane & 3u) + 4u -
              ((transform.tile_value_offset + chunk) & 3u)) & 3u;
            for (uint32_t i = first; i < min(128u, transform.coefficient_count - chunk); i += 4u) {
      ...
      if (lane == 0u) {
        for (uint32_t channel = 0u; channel < 2u; ++channel) {
          ...
          // CUDA fmin/fmax may turn NaN into a finite bound. Flag it before
          ...
          estimates[channel] -= fminf(fmaxf(step, -20.0f), 20.0f);
          if (fabsf(step) < 3.0e-3f) done[channel] = 1u;
        }
      }
      __syncthreads();
    ...
    const uint32_t color_index = tile_index;
    y_to_x[color_index] = QuantizeCfl(estimates[0]);
    y_to_b[color_index] = QuantizeCfl(estimates[1]);
    return;
}
...
__device__ __forceinline__ void EncodeResidentCoefficientsBody(
  ...
  for (uint32_t small = thread_index; !params.defer_dc && small < covered_count;
       small += blockDim.x) {
    const uint32_t x = small % batch.covered_width;
    const uint32_t y = small / batch.covered_width;
  ...
  if (params.defer_low_frequencies) return;
  __syncthreads();
...
__global__ void DcLowFrequenciesKernel(
    const CudaAqAnchor* anchors, const float* dc, float* coefficients,
    CudaAqExactBatch batch, uint32_t block_width, uint32_t block_height) {
  const uint32_t covered = batch.covered_width * batch.covered_height;
  const uint32_t task = blockIdx.x * blockDim.x + threadIdx.x;
  if (task >= batch.anchor_count * 3u * covered) return;
  const uint32_t anchor_index = task / (3u * covered);
  const uint32_t channel = (task / covered) % 3u;
  const uint32_t small = task % covered;
  const uint32_t u = small % batch.covered_width;
  const uint32_t v = small / batch.covered_width;
  const CudaAqAnchor anchor = anchors[batch.anchor_offset + anchor_index];
  const size_t area = size_t(block_width) * block_height;
  float value = 0.0f;
  for (uint32_t y = 0; y < batch.covered_height; ++y)
    for (uint32_t x = 0; x < batch.covered_width; ++x)
      value += dc[channel * area + size_t(anchor.y + y) * block_width + anchor.x + x] *
               ForwardBasis(batch.covered_height, v, y) *
               ForwardBasis(batch.covered_width, u, x);
  value *= UpsampleScale<true>(batch.covered_height, v) *
           UpsampleScale<true>(batch.covered_width, u);
  coefficients[batch.coefficient_offset +
    (channel * batch.anchor_count + anchor_index) * batch.coefficient_count +
    CoefficientIndex(batch, v, u)] = value;
}

}  // namespace

cudaError_t LaunchCudaAqDcLowFrequencies(
    const CudaAqAnchor* anchors, const float* dc, float* coefficients,
    CudaAqExactBatch batch, uint32_t block_width, uint32_t block_height,
    cudaStream_t stream) {
...
cudaError_t LaunchCudaAqFinalColorCorrelationNonlinear(
    const CudaAqColorTransformRecord* transforms, const uint32_t* tile_offsets,
    const float* quant_tables, const float* forward_coefficients,
    const int* raw_quant, const unsigned int* quantizer, signed char* y_to_x,
    signed char* y_to_b, unsigned int* error, uint32_t tile_count,
