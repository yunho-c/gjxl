...
Status ComputeCudaInputStoragePlan(Extent2D source, size_t* capacity_bytes) {
  if (capacity_bytes == nullptr || source.empty() ||
      source.width > std::numeric_limits<uint32_t>::max() ||
      source.height > std::numeric_limits<uint32_t>::max())
    return Status::InvalidArgument("CUDA input storage geometry is invalid");
  FrameGeometry geometry;
  Status status = FrameGeometry::Create(source, &geometry);
  if (!status.ok()) return status;
  const auto padded = geometry.padded_frame();
  if (padded.width > std::numeric_limits<uint32_t>::max() ||
      padded.height > std::numeric_limits<uint32_t>::max())
    return Status::InvalidArgument("CUDA input storage exceeds kernel dimensions");
  size_t capacity = 0;
  for (auto extent : {source, padded}) {
    for (size_t channel = 0; channel < 3; ++channel) {
      status = AddPlaneBytes(extent, &capacity);
      if (!status.ok()) return status;
    }
  }
  status = AddPlaneBytes({4, 1}, &capacity);
  if (!status.ok()) return status;
...
class CudaPreparedLinearRgbOpsin final : public PreparedGpuLinearRgbOpsin,
 ...
 public:
  ...
  Status Prepare(const ResidentInputPreparation& preparation) {
    ...
    const bool supplied = std::ranges::any_of(
      preparation.original_linear_rgb.plane, [](ConstPlaneF32View plane) {
        return plane.data != nullptr || plane.extent != Extent2D{} || plane.stride != 0;
      });
    if ((filling && supplied) || (!filling &&
        (preparation.fill_extent != Extent2D{} || preparation.fill_context != nullptr))) {
      return Status::InvalidArgument("CUDA resident input must specify exactly one source");
    }
    ConstImage3FView source = preparation.original_linear_rgb;
    if (filling) {
      FrameGeometry geometry;
      Status status = FrameGeometry::Create(preparation.fill_extent, &geometry);
      if (!status.ok()) return status;
      if (geometry.padded_frame() != preparation.coding_extent)
        return Status::InvalidArgument("CUDA generated input coding extent is invalid");
      generated_input_.resize(preparation.fill_extent);
      status = preparation.fill_original(preparation.fill_context, generated_input_.view());
      if (!status.ok()) return status;
      source = generated_input_.const_view();
    }
    return Prepare(source,
      {.padded_extent = preparation.coding_extent,
       .compute_matrix_scale_stats = preparation.compute_matrix_scale_statistics});
  }
  ...
  Status Prepare(
    ...
    if (!linear_rgb.valid() || !std::isfinite(options.intensity_target) ||
        ...
        !std::ranges::all_of(linear_rgb.plane, [](ConstPlaneF32View plane) {
          constexpr size_t limit = std::numeric_limits<size_t>::max() / sizeof(float);
          return plane.stride <= limit && plane.extent.width <= limit &&
            plane.extent.height - 1 <= (limit - plane.extent.width) / plane.stride;
        })) {
      return Status::InvalidArgument(
    ...
    size_t capacity = 0;
    status = ComputeCudaInputStoragePlan(linear_rgb.extent(), &capacity);
    if (!status.ok()) return status;
  ...
  ResidentInputStatistics statistics() const noexcept override {
    return {matrix_scale_stats_[0], matrix_scale_stats_[1], matrix_scale_stats_[2]};
  }

  ConstImage3FView original_linear_rgb_host() const noexcept override {
    return generated_input_.extent().empty() ? ConstImage3FView{}
                                           : generated_input_.const_view();
  }
 ...
 private:
  ...
  Image3FBuffer generated_input_;
...
Status CudaBackend::PrepareResidentInput(
  const ResidentInputPreparation& preparation,
  std::unique_ptr<PreparedResidentInput>* prepared) {
  if (prepared == nullptr)
    return Status::InvalidArgument("CUDA prepared resident input output is null");
  prepared->reset();
  const resource_budget_internal::ManagedHostScope resource_class(
  ...
  try {
    ...
    *prepared = std::move(candidate);
    ...
    return Status::OutOfMemory("Unable to allocate CUDA resident input");
  } catch (const std::length_error&) {
    return Status::InvalidArgument("CUDA resident input dimensions are too large");
  }
}
