...
class CudaPreparedExactAqEvaluation final : public PreparedAqEvaluation {
 public:
  CudaPreparedExactAqEvaluation() = default;
  ...
  Status Prepare(const AqEvaluationPreparation& preparation) {
    ...
    try {
      ...
      if (options_.profile.adaptive_dc_smoothing)
        smoothed_dc_staging_.resize(3 * block_count_);
    ...
    size_t persistent_bytes = 0, staging_bytes = 0;
    status = PlanArenas(&persistent_bytes, &staging_bytes);
    if (!status.ok()) return status;
 ...
 private:
  ...
  Status PlanArenas(size_t* persistent_out, size_t* staging_out) const {
  ...
  Status StageCoefficients(AqEvaluationInput input) {
    const VarDctEncoderFrame& frame = *input.exact_coefficients;
    ConstImage3FView dc = frame.dc();
    if (options_.profile.adaptive_dc_smoothing) {
      Image3FView smoothed;
      for (size_t c = 0; c < 3; ++c)
        smoothed.plane[c] = {smoothed_dc_staging_.data() + c * block_count_,
                             block_extent_, block_extent_.width};
      const Status status = SmoothDcCoefficients(dc, frame.quantizer(), smoothed);
      if (!status.ok()) return status;
      for (size_t c = 0; c < 3; ++c)
        dc.plane[c] = {smoothed.plane[c].data, block_extent_, block_extent_.width};
    }
  ...
  ManagedVector<float> coefficient_staging_;
  ManagedVector<float> smoothed_dc_staging_;
  ManagedVector<size_t> group_offsets_;
...
Status CudaPreparedExactAqEvaluation::ComputeStoragePlan(
    Extent2D source, const AqEvaluationOptions& evaluation,
    CudaCompatibilityStoragePlan* out) {
  if (out == nullptr) return Status::InvalidArgument("CUDA exact storage output is null");
  Status status = ValidateOptions(evaluation);
  if (!status.ok()) return status;
  FrameGeometry geometry;
  status = FrameGeometry::Create(source, &geometry);
  if (!status.ok()) return status;
  CudaPreparedExactAqEvaluation shape;
  shape.source_extent_ = source;
  shape.coding_extent_ = geometry.padded_frame();
  shape.block_extent_ = geometry.block_grid().blocks;
  if (!source.try_area(&shape.source_count_) ||
    ...
    return Status::InvalidArgument("CUDA exact storage exceeds coefficient indexing limits");
  shape.coefficient_count_ = 3 * shape.coding_count_;
  shape.options_ = evaluation;
  shape.filter_scratch_count_ = std::min<size_t>(2,
      size_t(evaluation.profile.loop_filter.gaborish) + evaluation.profile.loop_filter.epf_options.iterations);
  CudaCompatibilityStoragePlan plan;
  status = shape.PlanArenas(&plan.persistent_bytes, &plan.staging_bytes);
  if (!status.ok()) return status;
  if (evaluation.metric == AqEvaluationMetric::kButteraugli) {
    status = ComputeCudaButteraugliStoragePlan(source, &plan.butteraugli);
    if (!status.ok()) return status;
  }
  using enum resource_budget_internal::VectorCapacityPolicy;
  const size_t blocks = shape.block_count_;
  size_t groups = 0;
  if (!shape.block_extent_.ceil_div(kVarDctAcGroupBlockDimension).try_area(&groups))
    return Status::OutOfMemory("CUDA exact group storage overflows");
  // Reconfiguration can hold both current and replacement metadata. Grouped
  // anchors together hold at most one record per block, with vector growth.
  if (!plan.host.AddVector<uint8_t>(blocks, kFreshExact, 4) ||
      !plan.host.AddVector<HostAnchor>(blocks, kFreshExact, 2) ||
      !plan.host.AddVector<CudaAqAnchor>(blocks, kFreshExact) ||
      !plan.host.AddVector<CudaAqAnchor>(blocks, kGrowing) ||
      !plan.host.AddVector<float>(shape.coefficient_count_, kFreshExact) ||
      !plan.host.AddVector<size_t>(groups, kFreshExact) ||
      !plan.host.AddVector<float>(blocks, kFreshExact, 4) ||
      !plan.host.AddVector<float>(shape.source_count_, kFreshExact, 3) ||
      (evaluation.profile.adaptive_dc_smoothing &&
       !plan.host.AddVector<float>(blocks, kFreshExact, 6)))
    return Status::OutOfMemory("CUDA exact host storage overflows");
  *out = plan;
  return Status::Ok();
}

Status ComputeCudaExactStoragePlan(Extent2D source, const AqEvaluationOptions& evaluation,
                                 CudaCompatibilityStoragePlan* out) {
  return CudaPreparedExactAqEvaluation::ComputeStoragePlan(source, evaluation, out);
}
