...
Status ValidatePipelineInputs(
  ...
  if (!original_linear_rgb.valid() || (!resident_opsin && !opsin.valid()) ||
      (opsin.valid() && opsin.extent() != opsin_extent) ||
  ...
  *block_extent = BlockGrid::FromPaddedPixelExtent(opsin_extent).blocks;
...
Status quantization_pipeline_internal::PrepareResidentQuantizationPipeline(
  ...
  if (prepared == nullptr || !original_linear_rgb.valid() ||
      ...
      original_linear_rgb.width() <= padded_extent.width - kJxlBlockDimension ||
  ...
  try {
    ...
    candidate.epf_sharpness.resize(block_count);
    Status status = FillDefaultEpfSharpness({candidate.epf_sharpness.data(),
      candidate.block_extent,
      candidate.block_extent.width});
...
quantization_pipeline_internal::RunPreparedQuantizationPipelineWithProviders(
  ...
  const ConstImage3FView pipeline_opsin = initial_quantization_ready
    ? prepared.coding_opsin
    : prepared.preprocessed_opsin.const_view();
  const bool resident_opsin =
    initial_quantization_ready &&
    prepared.resident_coding_opsin.plane[0].buffer != nullptr;
  Extent2D block_extent;
  Status status = ValidatePipelineInputs(original_linear_rgb,
    pipeline_opsin,
  ...
  if (!prepared.preprocessing_ready ||
      ...
      (!resident_opsin && prepared.padded_extent != pipeline_opsin.extent()) ||
