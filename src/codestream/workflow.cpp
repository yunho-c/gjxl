...
Status EncodeNativeCodestream(
  ...
  if (profile == nullptr) {
    return EncodeVarDctCodestream(frame, gjxl_options, output);
  }
  ...
  const Status status = codestream_internal::EncodeVarDctCodestreamProfiled(
    frame, gjxl_options, output, &candidate.gjxl);
  if (!status.ok()) {
    return status;
  }
  candidate.backend =
...
[[nodiscard]] Status EncodePreparedAttempt(
  ...
  codestream_options.entropy_behavior =
    codestream_internal::ResolveEntropyBehavior(options);
  codestream_options.coefficient_order_behavior =
