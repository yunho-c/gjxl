...
void Same(std::span<const BitWriter> a, std::span<const BitWriter> b) {
...
void Compare(const Fixture& fixture, const EntropyCode& code, size_t budget,
  ...
  std::vector<BitWriter> expected(1);
  codestream_internal::Storage<BitWriter> actual(1);
          ...
          for (size_t budget : std::array<size_t, 4>{0, 1, 2, 8}) {
            ...
            for (unsigned fault = 0; fault < (mode ? 9u : 4u); ++fault) {
              ...
              switch (fault) {
                ...
                case 6: broken.ans_histograms[0].reverse_offsets.push_back(0); break;
