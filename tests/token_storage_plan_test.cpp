...
struct FrameFixture {
  ...
  bool Create(Extent2D extent, size_t family, int pattern) {
    ...
    if (!Ok(FrameGeometry::Create({extent.width * 8, extent.height * 8},
                                  &geometry)) ||
        !Ok(AcStrategyGrid::Create(extent, &strategies)) ||
    ...
    if (!Ok(chroma_from_luma_internal::CreateColorCorrelationMap(
      ...
      for (size_t gx = 0; gx < groups.width; ++gx) {
        ...
        for (size_t y = 0; y < local.height; ++y) {
          for (size_t x = 0; x < local.width; ++x) {
            ...
            if (!Ok(strategies.Set(gx * 32 + x, gy * 32 + y, strategy)))
              return false;
    ...
    return Check(view().valid(), "Synthetic tokenizer frame is invalid");
  }
