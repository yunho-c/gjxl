...
bool CompareCoefficientOracle(
  ...
  if (!CheckStatus(gjxl::Quantizer::Create(input.params, &quantizer),
                   "CPU oracle quantizer")) {
    return false;
  }
  gjxl::QuantizedCoefficientFrame frame;
  if (!CheckStatus(gjxl::ComputeQuantizedCoefficients(
                       image.View(), strategies, input.View().raw_quant_field,
                       quantizer, input.color, Options().coefficient_coding,
                       &frame),
                   "CPU coefficient oracle")) {
    return false;
  }
  if (frame.transforms().size() != snapshot.transforms.size()) {
    std::cerr << "Transform snapshot count mismatch\n";
    return false;
  }
  for (size_t transform_index = 0; transform_index < frame.transforms().size();
       ++transform_index) {
    const gjxl::QuantizedTransform &expected =
        frame.transforms()[transform_index];
    const auto &actual = snapshot.transforms[transform_index];
    if (actual.block_x != expected.block_x ||
        actual.block_y != expected.block_y ||
        actual.strategy != expected.strategy) {
      std::cerr << "Transform snapshot ordering mismatch\n";
      return false;
    }
    for (size_t channel = 0; channel < 3; ++channel) {
      if (actual.quantized_coefficients[channel] != expected.ac[channel]) {
        const auto mismatch =
            std::mismatch(actual.quantized_coefficients[channel].begin(),
                          actual.quantized_coefficients[channel].end(),
                          expected.ac[channel].begin());
        std::cerr << "Quantized coefficient mismatch for transform "
                  << transform_index << ", channel " << channel << ", index "
                  << (mismatch.first -
                      actual.quantized_coefficients[channel].begin())
                  << '\n';
        return false;
      }
    }
  ...
  for (size_t channel = 0; channel < 3; ++channel) {
    for (size_t y = 0; y < kBlockExtent.height; ++y) {
      for (size_t x = 0; x < kBlockExtent.width; ++x) {
        ...
        if (!Near(snapshot.dc[channel][index], frame.dc(channel, x, y), 2.0e-4,
                  5.0e-5)) {
  ...
  if (!CheckStatus(gjxl::ReconstructQuantizedCoefficients(
                       ...
                       Options().coefficient_coding, output),
