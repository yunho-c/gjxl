...
gjxl::AqEvaluationOptions MakeOptions() {
  ...
  options.profile.intensity_target = 255.0f;
...
float OptionProbeValue(const gjxl::AqEvaluationOptions& options) {
  float value = gjxl::QuantizationMatrixMultiplier(options.profile.x_qm_scale) *
    0.0625f;
  value += gjxl::QuantizationMatrixMultiplier(options.profile.b_qm_scale) *
    0.03125f;
...
bool CheckPublicAndPreparationValidation(
  ...
  bad_options.profile.loop_filter.epf_options.pass0_sigma_scale =
