  ...
  try {
    ...
    for (gjxl::Extent2D blocks : std::array<gjxl::Extent2D, 23>{{
      ...
      for (const auto& stage : gjxl::ac_strategy_internal::kCandidateStages) {
        ...
        if (stage.anchor_step > 1) {
          // Effort 10 also considers odd DCT32-family anchors. Enumerate them
          // in this scalar fixture, independently of the production generator,
          // then compare each fused channel loss/rate against separate kernels.
          auto dense_stage = stage;
          dense_stage.anchor_step = 1;
          for (const bool device_cfl : {false, true})
            Case(*gpu, blocks, dense_stage, device_cfl);
        }
      }
    }
