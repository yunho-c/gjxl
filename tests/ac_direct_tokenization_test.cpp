...
void CheckRejectedGroup(
  ...
  const auto status = gjxl::codestream_internal::TokenizeSimpleAcGroupForEncoder(
    gjxl::vardct_frame_internal::BorrowFrame(frame), {}, natural, map, frame.ac_group_count(), true, scratch, &output);
  Require(!status.ok() && output.values == gjxl::codestream_internal::Storage<uint32_t>{123} &&
            output.contexts == gjxl::codestream_internal::Storage<uint16_t>{9} &&
            ...
            for (size_t group = 0; group < reference.size(); ++group) {
              ...
              for (const bool collect : {true, false}) {
                ...
                Check(gjxl::codestream_internal::TokenizeSimpleAcGroupForEncoder(
                  gjxl::vardct_frame_internal::BorrowFrame(frame), order, natural, map, group, collect, &scratch, &actual));
