...
| Direct balanced ANS | One private H source array with an in-place Shannon cache; clustered/reordered K histograms and canonicalization scratch; two C maps; K fixed populations; one final width model and all configuration/table/writer scratch. Validated fixed populations without an initial map use small borrowed views; scanned or preclustered input owns counts. The bound conservatively covers either source form with owning histogram sizes; caller-owned input backing is excluded. |
