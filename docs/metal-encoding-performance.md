...
stable fully-resident submission IDs are `frontend.prepare_aq.reference`,
`frontend.initial_quantization`, `frontend.ac_strategy`,
`frontend.quant_adjustment`, and `resident.aq`. Reference preparation now
occurs when the single complete resident evaluator is constructed before AC
search; after search, that compatible evaluator reports
`frontend.reconfigure_aq` instead of a second `frontend.prepare_aq` span. The
resident evaluator folds preprocessing and initial CfL into
