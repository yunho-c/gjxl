...
public:
  ...
  Status GpuProfile(GpuSubmissionProfile* profile) {
    ...
    if (g_control == OverheadControl::Graph || g_control == OverheadControl::Split) {
...
Status MetalBackend::SubmitCompute(
  ...
  OverheadTimer encoding_timer{g_overhead.encode_ns};
  ++g_overhead.encoders;
...
Status MetalBackend::SubmitComputeProfiled(
  ...
  MTL::CounterSampleBuffer* raw_sample_buffer = nullptr;
  if (g_control != OverheadControl::Record) {
  ...
  for (size_t index = 0; index < stages.size(); ++index) {
    ...
    MTL::ComputeCommandEncoder* encoder = nullptr;
    if (mode == GpuProfilingMode::kStage && g_control != OverheadControl::Record) {
