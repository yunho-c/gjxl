...
bool CheckStatus(
  ...
  { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
}
...
bool CheckSubmissionStorage(gjxl::GpuBackend& gpu,
  ...
  if (!profiler || !resolver) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  ...
  if (!CheckStatus(ComputeAcSubmissionStoragePlan({3, 2, true}, &plan),
                   "Plan AC submission")) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  for (const auto mode : {GpuProfilingMode::kStage, GpuProfilingMode::kDispatch}) {
    ...
    if (!budget.TryReserve(plan.working.peak_bytes, &job).ok()) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    {
      ...
      if (!CheckStatus(profiler->EvaluateAcStrategyCandidateBatchesProfiled(
          ...
          !CheckStatus(resolver->ResolveGpuSubmissionProfile(*submission,
                       ...
                       "Resolve AC profile")) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    }
    if (result.submissions.size() != 1 ||
        ...
        budget.snapshot().total.live_capacity_bytes >
          plan.profile.resolved_output.retained_bytes) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    size_t dispatches = 0;
      ...
      if (stage.dispatches.size() == 2) {
        ...
        switch (batch.strategy) {
          ...
          case AcStrategyType::kDct8x16:
            ...
            break;
          default: { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
        }
        if (stage.dispatches.front().kernel_id != expected_kernel ||
            stage.dispatches.back().kernel_id !=
              "gjxl_ac_strategy_cost_from_loss") { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
      }
      dispatches += stage.dispatches.size();
    }
    if (dispatches > plan.maximum_dispatches) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    ...
    if (budget.snapshot().committed_bytes() != 0) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  }
  if (batch.strategy == AcStrategyType::kDct8) {
    ...
    for (size_t position = 0; position < 3; ++position) {
      ...
      if (!budget.TryReserve(plan.working.peak_bytes, &job).ok()) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
      {
        ...
        if (status.code() != StatusCode::kOutOfMemory ||
            ...
            !submission || !submission->Wait().ok()) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
      }
      job.Reset();
      if (budget.snapshot().committed_bytes() != 0) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    }
    AcSubmissionStoragePlan validation;
    if (!ComputeAcSubmissionStoragePlan({3}, &validation).ok()) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    for (size_t capacity : {size_t{0}, validation.input.peak_bytes,
      ...
      if (!budget.TryReserve(std::max(size_t{1}, capacity), &job).ok() ||
          (capacity == 0 && !job.ReduceCapacity(0).ok())) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
      {
        ResourceContextScope scope({&job, ResourceClass::kPreparation});
        std::unique_ptr<GpuSubmission> submission;
        const auto before = gpu.stats().committed_submissions;
        ...
        if (!status.resource_plan_exceeded() || submission ||
            gpu.stats().committed_submissions != before) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
      }
      job.Reset();
      if (budget.snapshot().committed_bytes() != 0) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    }
...
bool CheckCompactScratchRanges(gjxl::GpuBackend& gpu,
  ...
  for (const size_t count : {1, 8, 11, 16, 32, 33}) {
    ...
    if (scratch.scratch_b_bytes != 0) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    ...
    if (!Allocate(gpu, arena_bytes, "Allocate compact arena", &arena) ||
        !CheckStatus(gpu.CopyHostToDevice(*arena, words.data(), arena_bytes),
          "Initialize compact guards")) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    auto compact = batch;
    ...
    if (!CheckStatus(gjxl::EvaluateAcStrategyCandidateBatches(
        ...
        !CheckStatus(gpu.CopyDeviceToHost(*arena, words.data(), arena_bytes),
          "Read compact arena")) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    ...
    for (size_t i = 0; i < sizes.size(); ++i) {
      ...
      if (!Rejected(invalid)) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    ...
    if (!Rejected(input_alias)) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  }
  ...
  if (!CheckStatus(gjxl::CreateCudaBackend(&foreign_gpu), "Create foreign backend") ||
      !Allocate(*foreign_gpu, batch.candidate_count * 3 * sizeof(float),
        "Allocate foreign compact scratch", &foreign_a)) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  auto foreign = batch;
  ...
  if (!CheckStatus(gjxl::EvaluateAcStrategyCandidates(gpu, unused_b,
      ...
      !CheckStatus(submission->Wait(), "Wait unused B fields")) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  ...
  if (!CheckStatus(gjxl::EvaluateAcStrategyCandidates(gpu, conservative,
      ...
      !CheckStatus(submission->Wait(), "Wait conservative scratch")) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
...
bool RunStrategyCase(
  ...
  gjxl::AcStrategyScratchRequirements scratch;
  if (!CheckStatus(gjxl::GetAcStrategyScratchRequirements(
        gpu, strategy, candidates.size(), &scratch), "Query scratch")) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
...
#ifdef GJXL_TEST_CUDA
  if (scratch.rate_scratch_bytes != rate_bytes || scratch.scratch_b_bytes != 0 ||
      scratch.scratch_a_bytes != candidates.size() * 3 * sizeof(float)) {
    std::cerr << "Unexpected CUDA AC scratch requirements\n";
    { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  }
#endif
  ...
  for (unsigned slot = 0; slot < 2; ++slot) {
    ...
    if (!Allocate(gpu, bytes - 1, "Allocate undersized scratch", &short_buffer)) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  ...
  if (!CheckSubmissionStorage(gpu, batch, poisoned_costs)) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
#endif
  ...
  if (!CheckCompactScratchRanges(gpu, batch, complete_costs)) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
...
bool CheckImplementation(
  ...
  for (const gjxl::AcStrategyType strategy : kStrategies) {
    if (!RunStrategyCase(*gpu, name, strategy, fixture)) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  }
  return true;
}
#endif
...
bool CheckValidation() {
  ...
  for (auto strategy : kStrategies) {
    ...
    if (!gjxl::GetAcStrategyScratchRequirements(*gpu, strategy, 0, &sizes).ok() ||
        sizes.scratch_a_bytes || sizes.scratch_b_bytes || sizes.rate_scratch_bytes) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    sizes = {1, 2, 3};
    if (gjxl::GetAcStrategyScratchRequirements(*gpu, strategy,
        ...
        sizes.rate_scratch_bytes != 3) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  }
  gjxl::AcStrategyScratchRequirements sizes{1, 2, 3};
  if (gjxl::GetAcStrategyScratchRequirements(*gpu, gjxl::AcStrategyType::kCount, 1, &sizes).ok() ||
      ...
      gpu->stats().committed_submissions != before_query.committed_submissions) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
#ifdef GJXL_TEST_CUDA
  {
  const auto before_query = gpu->stats();
  for (const auto strategy : kStrategies) {
    ...
    if (!CheckStatus(gjxl::GetAcStrategyScratchRequirements(
        ...
        scratch.rate_scratch_bytes != 0) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    scratch = {1, 2, 3};
    if (gjxl::GetAcStrategyScratchRequirements(*gpu, strategy,
          std::numeric_limits<size_t>::max(), &scratch).code() !=
          gjxl::StatusCode::kInvalidArgument ||
        scratch.scratch_a_bytes != 1 || scratch.scratch_b_bytes != 2 ||
        scratch.rate_scratch_bytes != 3) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
  }
...
bool CheckLossFusionExact(const Fixture& fixture) {
  ...
  for (size_t index = 0; index < modes.size(); ++index) {
    ...
    if (!CheckStatus(gjxl::CreateMetalBackend(GJXL_METALLIB_PATH,
          ...
          &gpu), "Create exact loss-fusion comparison backend")) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    gjxl::ac_strategy_search_internal::StoragePlan plan;
    if (!gjxl::ac_strategy_search_internal::ComputeStoragePlan(
          {3840, 2160}, true, &plan, gpu.get()).ok()) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    ...
    for (auto strategy : kStrategies) {
      if (!RunStrategyCase(*gpu, "exact loss fusion", strategy, fixture,
                           &costs[index])) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
    }
  }
  if (costs[0].size() != costs[1].size()) { std::cerr << "Diagnostic false at source line " << __LINE__ << '\n'; return false; }
