    ...
    for batch in map(int,a.batches.split(',')):
     ...
     for mode in modes:
      ...
      if done.exists():record=json.loads(done.read_text())
       ...
       env={k:v for k,v in os.environ.items() if not k.startswith('GJXL_EXPERIMENT_')};env['GJXL_EXPERIMENT_DC_WORKERS']='8'
       ...
       (case/f'command{attempt}.json').write_text(json.dumps({'argv':command,'environment':{k:v for k,v in env.items() if k.startswith('GJXL_EXPERIMENT_')},'input_sha256':im['input_sha256']},indent=2))
