#!/usr/bin/env python3
"""Compare a frozen pre-fusion encoder with native and fused resident selection."""
import argparse
import json
import os
...
def main():
    ...
    binaries = {'old': output / 'control', 'new': output / 'candidate'}
    shutil.copy2(args.control_binary, binaries['old'])
    shutil.copy2(ROOT / 'build/frontier/gjxl_quality_benchmark', binaries['new'])
    ...
    sources += ['tools/frontier_dp/time_aq_initialization.py']
    for name in sources:
    ...
    manifest = dict(revision=subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
                    ...
                    control_binary=str(args.control_binary.resolve()),
                    protocol='Complete-call wall time; efforts 5/8 at distance 1.2; 8 threads; '
                             ...
                             'old CPU selector, old native GPU selector, new GPU selector plus fused AQ initialization; '
    ...
    for repetition in range(3):
        for index, (name, effort) in enumerate(cases[repetition:] + cases[:repetition]):
            ...
            for arm in arms[shift:] + arms[:shift]:
                ...
                binary = binaries['new' if arm == 'fused' else 'old']
    ...
    for name, effort in cases:
        ...
        item['combined_speedup'] = item['cpu']['ms'] / item['fused']['ms']
        item['initialization_speedup'] = item['gpu']['ms'] / item['fused']['ms']
        summary.append(item)
