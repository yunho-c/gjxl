#!/usr/bin/env python3
"""Paired complete-encode timing of native selection and CPU-policy control."""
import argparse
import json
import os
...
def main():
    ...
    binary = output / 'gjxl_quality_benchmark'
    shutil.copy2(ROOT / 'build/frontier/gjxl_quality_benchmark', binary)
    ...
    manifest = dict(revision=subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
                    ...
                    protocol='Complete-call wall time; efforts 5/8 at distance 1.2; 8 threads; '
                             '3 processes, 3 warmups, 7 samples per arm/case; rotated pair order; '
                             'same binary, CPU control through experiment override; byte identity required')
    ...
    for repetition in range(3):
        for index, (name, effort) in enumerate(cases[repetition:] + cases[:repetition]):
            arms = ['gpu', 'cpu'] if (repetition + index) % 2 == 0 else ['cpu', 'gpu']
            for arm in arms:
                folder = output / f'{name}-e{effort}-{arm}-{repetition}'
                folder.mkdir()
                selected_env = env if arm == 'gpu' else {**env, 'GJXL_AC_SEARCH_EXPERIMENT': 'greedy'}
    ...
    for name, effort in cases:
        ...
        item['speedup'] = item['cpu']['ms'] / item['gpu']['ms']
        summary.append(item)
