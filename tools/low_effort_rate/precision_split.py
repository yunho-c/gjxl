#!/usr/bin/env python3
"""Separate extra DC precision and prediction-aware rounding along supported settings."""
...
def main():
    ...
    manifest = {'files': files, 'image_order': image_ids, 'qualities': c.m['qualities'],
                'interventions': 'ordinary rounding + extra bit 1; compare ordinary bit 0 and prediction-aware bit 1 from prior factorial; smoothing off; efforts 3 and 4',
                'expected': 70, 'base_manifest_sha256': study.sha(base / 'manifest.json')}
    ...
    with (root / 'run.lock').open('a') as lock:
        ...
        try:
            for name in image_ids:
                ...
                for e, mode, quality in itertools.product((3,4), ('round',), manifest['qualities']):
            ...
            comparisons = []
            for name,e,pair in itertools.product(image_ids,(3,4),(('round',0),('round',1),('prediction-aware',1))):
                mode, precision = pair
            ...
            summary = []
            for e,pair in itertools.product((3,4),(('round',0),('round',1),('prediction-aware',1))):
                mode, precision = pair
